...
Так же класс имеет сеттер для сохранения данных счетчика.
...
Так же класс имеет сеттер для сохранения данных каталога.
...
Конструктор класса приниимает корневой элемент модального окна и экземпляр класса `EventEmitter` для возможности инициации события.
В полях класса:
...
А так же сеттер для сохранения данных контента.
...
Поля класса содержат элементы разметки элементов карточки. Конструктор, кроме темплейта принимает экземпляр `EventEmitter` для инициации событий и опционально объект с колбэком для вызова при клике.\
